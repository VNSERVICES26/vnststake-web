...
// Contract addresses
const STAKING_CONTRACT = "0x94bC53fD2D7915fD70A120eBfb96bB99A81fFfeF";
const VNST_TOKEN = "0x37c401DdBC8030c63BF3B3b2816Ba379BfD10474";
const VNT_TOKEN = "0xa7e41CB0A41dbFC801408d3B577fCed150c4eeEc";
const USDT_TOKEN = "0x337610d27c682E347C9cD60BD4b3b107C9d34dDd";
const ADMIN_ADDRESS = "0xe63bd9887504B76BdE5bD9A5145fcb9aAAadadAF"; // Will be used for referral logic

// Minimum and maximum staking amounts (update as needed)
const MIN_STAKE = 100;
const MAX_STAKE = 10000;

// Load contract ABI (simplified placeholder, replace with actual ABI)
...
async function initWeb3() {
  if (window.ethereum) {
    ...
    try {
      ...
      document.getElementById("walletConnectBtn").innerText = accounts[0].slice(0, 6) + "..." + accounts[0].slice(-4);
      loadContracts();
    } catch (err) {
...
async function loadContracts() {
  stakingContract = new web3.eth.Contract(stakingAbi, STAKING_CONTRACT);
  vnstToken = new web3.eth.Contract(erc20Abi, VNST_TOKEN);

  // Update user data if on staking page
...
async function updateUserData() {
  const balance = await vnstToken.methods.balanceOf(accounts[0]).call();
  document.getElementById("vnstBalance").innerText = web3.utils.fromWei(balance);

  // Add other dynamic updates: referral info, active stake, direct count, etc.
}

async function approveMax() {
  const maxAmount = web3.utils.toWei("1000000000");
  await vnstToken.methods.approve(STAKING_CONTRACT, maxAmount).send({ from: accounts[0] });
  alert("Approved maximum staking amount");
}

async function stakeVNST() {
  const refInput = document.getElementById("referralAddress").value;
  const amountInput = document.getElementById("stakeAmount").value;
  const referral = refInput || ADMIN_ADDRESS;
  ...
  const allowance = await vnstToken.methods.allowance(accounts[0], STAKING_CONTRACT).call();

  if (BigInt(allowance) < BigInt(amount)) {
    alert("Please approve maximum amount first");
    return;
  }

  // Call staking function
  try {
    await stakingContract.methods.stake(amount, referral).send({ from: accounts[0] });
...
async function claimVNT() {
  try {
    await stakingContract.methods.claimVNT().send({ from: accounts[0] });
    alert("VNT Claimed!");
  } catch (err) {
    console.error(err);
    alert("Claim failed");
...
async function claimUSDT() {
  try {
    await stakingContract.methods.claimUSDT().send({ from: accounts[0] });
    alert("USDT Claimed!");
  } catch (err) {
    console.error(err);
    alert("Claim failed");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (approveBtn) approveBtn.addEventListener("click", approveMax);
